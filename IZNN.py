...
def sgnq(e, q):
    return np.where(e > 0, np.abs(e)**q, np.where(e < 0, -np.abs(e)**q, 0))

def sgn_invq(e, q):
    return np.where(e > 0, np.abs(e)**(1/q), np.where(e < 0, -np.abs(e)**(1/q), 0))

def sgn(e):   #using q=1/2
    return sgnq(e, 1/2) + sgn_invq(e, (1/2)**(-1))
...
def equation(t, state):
    ...
    dx11 = -gamma*(np.sin(t)*(sgn(x11*np.sin(t)+x21*np.cos(t)-1))-np.cos(t)*(sgn(x21*np.sin(t)-x11*np.cos(t))))+x21
    dx12 = -gamma*(np.sin(t)*(sgn(x12*np.sin(t)+x22*np.cos(t)))-np.cos(t)*(sgn(x22*np.sin(t)-x12*np.cos(t)-1)))+x22
    dx21 = -gamma*(np.cos(t)*(sgn(x11*np.sin(t)+x21*np.cos(t)-1))+np.sin(t)*(sgn(x21*np.sin(t)-x11*np.cos(t))))-x11
    dx22 = -gamma*(np.cos(t)*(sgn(x12*np.sin(t)+x22*np.cos(t)))+np.sin(t)*(sgn(x22*np.sin(t)-x12*np.cos(t)-1)))-x12
...
time_bound = 10000
plot_time = np.linspace(0, time_bound, 10000)


X_Y_Spline = make_interp_spline(plot_time, error[0: 10*time_bound])
